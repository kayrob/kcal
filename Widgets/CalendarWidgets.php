    ...
    */
    public function get_calendar_events_ajax($get){
        ...
        if (isset($get["calendar"]) && isset($get["start"]) && isset($get["end"])){
            ...
            if ($res !== false){
                $rows = $this->retrieve_all_events($get["start"], $get["end"], $get["calendar"]);
                if (false !== $rows){
                    $eList = array();
    ...
    public function buildRSS($calendarID, $limit){
        ...
        if ((int) $calendarID > 0 || $calendarID === false){
           ...
           if (count($res) > 0) {
               ...
               foreach($res as $row) {
                    $startTimeStamp = strtotime($row->eventStartDate);
                    $pubTimeStamp = strtotime($row->eventStartDate);
                    if (array_key_exists(trim($row->calendarID), $calendars) && $startTimeStamp >= (date("U"))) {
                        $link = "http://".$_SERVER['SERVER_NAME']. $this->url ."?event=".trim($row->itemID)."&start=".$startTimeStamp;
                        $source = false;
                        if (!empty($row->detailsAlternateURL)){
                        ...
                        $rss .= "<link>" .$link. "</link>"."\n";
                        $rss .= "<description><![CDATA[" .trim($row->description). "]]></description>"."\n";
                        $rss .= "<pubDate>@" .$pubTimeStamp. "</pubDate>"."\n";
